...
def val(model):
    ...
    for data in tqdm(dataloader):
        with torch.no_grad():
            ...
            for img_id, ratio, detection, size, offset in zip(img_ids, ratios, detections, sizes, offsets):
                bbox = detection[:, :4]
                prob = detection[:, 4]
                label = detection[:, 5].astype(np.int32) + 1

                bbox[:, 0::2] *= size[0].item()
                bbox[:, 1::2] *= size[1].item()
                bbox[:, 0::2] -= offset[0]
                bbox[:, 1::2] -= offset[1]

                predictions.update({img_id: [ratio, bbox, prob, label]})
